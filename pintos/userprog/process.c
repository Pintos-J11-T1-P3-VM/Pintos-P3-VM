...
static bool lazy_load_segment(struct page* page, void* aux)
{
    ...
    struct args_load_segment* args = (struct args_load_segment*)aux;
    bool lock_held = lock_held_by_current_thread(&filesys_lock);
    if (!lock_held)
        lock_acquire(&filesys_lock);
    file_seek(args->file, args->offset);
    int read_bytes = file_read(args->file, page->frame->kva, args->read_bytes);
    if (read_bytes != args->read_bytes) {
        if (!lock_held)
            lock_release(&filesys_lock);
        return false;
    }
    ...
    if (!lock_held)
        lock_release(&filesys_lock);
    memset(page->frame->kva + args->read_bytes, 0, PGSIZE - args->read_bytes); // dst, value, size
    return true;
}
                         ...
                         bool writable)
{
    ...
    while (read_bytes > 0 || zero_bytes > 0) {
        ...
        struct args_load_segment* aux = malloc(sizeof(struct args_load_segment)); // todo: free
...
static bool setup_stack(struct intr_frame* if_)
{
    ...
    /* TODO: Your code goes here */
    if (vm_alloc_page(VM_MARKER_0 | VM_ANON, stack_bottom, true)) // type: stack and anon
